...
def get_info_about_all_cards(operations: list[dict]) -> dict:
    ...
    for key in count.keys():
        if isinstance(key, str):
            var = sum(operation.get('Сумма операции с округлением', 0) for operation in operations if operation.get(f'Номер карты') == key)
            cards['cards'].append({
                "last_digits": key[1:],
                "total": round(var, 2),
...
def get_top_five_transactions(operations: list[dict]) -> dict:
    ...
    try:
        ...
        sorted_by_amount = sorted(operations, key=lambda x: x.get('Сумма операции с округлением' , ''), reverse=True)
...
def get_exchange_rate(currencies: list,) -> dict:
    ...
    try:
        for currency in currencies:
            ...
            if 200 <= status <= 300:
                rate = round(float(data['Realtime Currency Exchange Rate']['5. Exchange Rate']), 2)
...
def get_days_by_date(operations:list[dict], date: str) -> list[dict]:
    ...
    day = date.split(".")[0]
    year = date.split(".")[2]
    year_operations = [operation for operation in operations if
    ...
    days_operations = [operation for operation in month_operations if
                       int(operation.get('Дата операции', '').split(" ")[0].split('.')[0]) <= int(day)]
